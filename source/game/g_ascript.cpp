...
static qboolean objectTrace_doTrace( asvec3_t *start, asvec3_t *mins, asvec3_t *maxs, asvec3_t *end, int ignore, int contentMask, astrace_t *self )
{
	...
	return ( self->trace.ent != -1 ) ? qtrue : qfalse;
}
...
static qboolean objectGameClient_isReady( gclient_t *self )
{
	...
	return ( level.ready[self - game.clients] || GS_MatchState() == MATCH_STATE_PLAYTIME ) ? qtrue : qfalse;
}
...
static qboolean objectGameClient_isBot( gclient_t *self )
{
	...
	return ( ( game.edicts[ playerNum + 1 ].r.svflags & SVF_FAKECLIENT ) &&
		game.edicts[ playerNum + 1 ].ai.type == AI_ISBOT ) ? qtrue : qfalse;
}
...
static qboolean objectGameEntity_IsGhosting( edict_t *self )
{
	...
	return G_ISGHOSTING( self ) ? qtrue : qfalse;
}
...
{
	{ "cEntity @G_SpawnEntity( const String &in )", asFUNCTION(asFunc_G_Spawn) },
	{ "String @G_SpawnTempValue( const String &in )", asFUNCTION(asFunc_G_SpawnTempValue) },
	{ "cEntity @G_GetEntity( int entNum )", asFUNCTION(asFunc_GetEntity) },
	{ "cClient @G_GetClient( int clientNum )", asFUNCTION(asFunc_GetClient) },
	{ "cTeam @G_GetTeam( int team )", asFUNCTION(asFunc_GetTeamlist) },
	{ "cItem @G_GetItem( int tag )", asFUNCTION(asFunc_GS_FindItemByTag) },
	{ "cItem @G_GetItemByName( const String &in name )", asFUNCTION(asFunc_GS_FindItemByName) },
	{ "cItem @G_GetItemByClassname( const String &in name )", asFUNCTION(asFunc_GS_FindItemByClassname) },
	{ "cEntity @G_FindEntityInRadius( cEntity @, cEntity @, const Vec3 &in, float radius )", asFUNCTION(asFunc_FindEntityInRadiusExt) },
	{ "cEntity @G_FindEntityInRadius( cEntity @, const Vec3 &in, float radius )", asFUNCTION(asFunc_FindEntityInRadius) },
	{ "cEntity @G_FindEntityWithClassname( cEntity @, const String &in )", asFUNCTION(asFunc_FindEntityWithClassname) },
	{ "cEntity @G_FindEntityWithClassName( cEntity @, const String &in )", asFUNCTION(asFunc_FindEntityWithClassname) },

	// misc management utils
	{ "void G_RemoveAllProjectiles()", asFUNCTION(asFunc_G_Match_RemoveAllProjectiles) },
	{ "void G_RemoveDeadBodies()", asFUNCTION(asFunc_G_Match_FreeBodyQueue) },
	{ "void G_Items_RespawnByType( uint typeMask, int item_tag, float delay )", asFUNCTION(asFunc_G_Items_RespawnByType) },

	// misc
	{ "void G_Print( const String &in )", asFUNCTION(asFunc_Print) },
	{ "void G_PrintMsg( cEntity @, const String &in )", asFUNCTION(asFunc_PrintMsg) },
	{ "void G_CenterPrintMsg( cEntity @, const String &in )", asFUNCTION(asFunc_CenterPrintMsg) },
	{ "void G_Sound( cEntity @, int channel, int soundindex, float attenuation )", asFUNCTION(asFunc_G_Sound) },
	{ "void G_PositionedSound( const Vec3 &in, int channel, int soundindex, float attenuation )", asFUNCTION(asFunc_PositionedSound) },
	{ "void G_GlobalSound( int channel, int soundindex )", asFUNCTION(asFunc_G_GlobalSound) },
	{ "void G_LocalSound( cClient @, int channel, int soundIndex )", asFUNCTION(asFunc_G_LocalSound) },
	{ "void G_AnnouncerSound( cClient @, int soundIndex, int team, bool queued, cClient @ )", asFUNCTION(asFunc_G_AnnouncerSound) },
	{ "float random()",asFUNCTION(asFunc_Random) },
	{ "float brandom( float min, float max )", asFUNCTION(asFunc_BRandom) },
	{ "int G_DirToByte( const Vec3 &in origin )", asFUNCTION(asFunc_DirToByte) },
	{ "int G_PointContents( const Vec3 &in origin )", asFUNCTION(asFunc_PointContents) },
	{ "bool G_InPVS( const Vec3 &in origin1, const Vec3 &in origin2 )", asFUNCTION(asFunc_InPVS) },
	{ "bool G_WriteFile( const String &, const String & )", asFUNCTION(asFunc_WriteFile) },
	{ "bool G_AppendToFile( const String &, const String & )", asFUNCTION(asFunc_AppendToFile) },
	{ "String @G_LoadFile( const String & )", asFUNCTION(asFunc_LoadFile) },
	{ "int G_FileLength( const String & )", asFUNCTION(asFunc_FileLength) },
	{ "void G_CmdExecute( const String & )", asFUNCTION(asFunc_Cmd_ExecuteText) },
	{ "String @G_LocationName( const Vec3 &in origin )", asFUNCTION(asFunc_LocationName) },
	{ "int G_LocationTag( const String & )", asFUNCTION(asFunc_LocationTag) },
	{ "String @G_LocationName( int tag )", asFUNCTION(asFunc_LocationForTag) },
	...
	{ "int G_ImageIndex( const String &in )", asFUNCTION(asFunc_ImageIndex) },
	{ "int G_SkinIndex( const String &in )", asFUNCTION(asFunc_SkinIndex) },
	{ "int G_ModelIndex( const String &in )", asFUNCTION(asFunc_ModelIndex) },
	{ "int G_SoundIndex( const String &in )", asFUNCTION(asFunc_SoundIndex) },
	...
	// projectile firing
	{ "void G_FireInstaShot( const Vec3 &in origin, const Vec3 &in angles, int range, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireInstaShot) },
	{ "cEntity @G_FireWeakBolt( const Vec3 &in origin, const Vec3 &in angles, int speed, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireWeakBolt) },
	{ "void G_FireStrongBolt( const Vec3 &in origin, const Vec3 &in angles, int range, int damage, int knockback, int stun, cEntity @owner )",asFUNCTION( asFunc_FireStrongBolt) },
	{ "cEntity @G_FirePlasma( const Vec3 &in origin, const Vec3 &in angles, int speed, int radius, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FirePlasma) },
	{ "cEntity @G_FireRocket( const Vec3 &in origin, const Vec3 &in angles, int speed, int radius, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireRocket) },
	{ "cEntity @G_FireGrenade( const Vec3 &in origin, const Vec3 &in angles, int speed, int radius, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireGrenade) },
	{ "void G_FireRiotgun( const Vec3 &in origin, const Vec3 &in angles, int range, int spread, int count, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireRiotgun) },
	{ "void G_FireBullet( const Vec3 &in origin, const Vec3 &in angles, int range, int spread, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireBullet) },
	{ "cEntity @G_FireBlast( const Vec3 &in origin, const Vec3 &in angles, int speed, int radius, int damage, int knockback, int stun, cEntity @owner )", asFUNCTION(asFunc_FireBlast) },

	{ "bool ML_FilenameExists( String & )", asFUNCTION(asFunc_ML_FilenameExists) },
	{ "String @ML_GetMapByNum( int num )", asFUNCTION(asFunc_ML_GetMapByNum) },
...
{
	game.asEngine = NULL;
	game.asGlobalsInitialized = qfalse;
...
static void G_InitializeGameModuleSyntax( asIScriptEngine *asEngine )
{
	...
	game.asGlobalsInitialized = qtrue;
