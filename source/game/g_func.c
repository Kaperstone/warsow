...
static qboolean Move_AdjustFinalStep( edict_t *ent )
{
...
static qboolean AngleMove_AdjustFinalStep( edict_t *ent )
{
	float movedist, remainingdist;
	vec3_t destdelta;
...
static void AngleMove_Begin( edict_t *ent )
{
	...
	// set up velocity vector
	VectorSubtract( ent->moveinfo.destangles, ent->s.angles, destdelta );
	dist = VectorNormalize( destdelta );
	VectorScale( destdelta, ent->moveinfo.speed, ent->avelocity );
	ent->nextThink = level.time + 1;
	ent->think = AngleMove_Watch;
}
...
static void plat_hit_top( edict_t *ent )
{
	if( !( ent->flags & FL_TEAMSLAVE ) )
	{
		if( ent->moveinfo.sound_end )
			G_AddEvent( ent, EV_PLAT_HIT_TOP, ent->moveinfo.sound_end, qtrue );
		ent->s.sound = 0;
	}
...
static void plat_hit_bottom( edict_t *ent )
{
	...
	{
		if( ent->moveinfo.sound_end )
			G_AddEvent( ent, EV_PLAT_HIT_BOTTOM, ent->moveinfo.sound_end, qtrue );
		ent->s.sound = 0;
	}
...
void plat_go_down( edict_t *ent )
{
	if( !( ent->flags & FL_TEAMSLAVE ) )
	{
		if( ent->moveinfo.sound_start )
			G_AddEvent( ent, EV_PLAT_START_MOVING, ent->moveinfo.sound_start, qtrue );
...
static void door_hit_top( edict_t *self )
{
	if( !( self->flags & FL_TEAMSLAVE ) )
	{
		if( self->moveinfo.sound_end )
			G_AddEvent( self, EV_DOOR_HIT_TOP, self->moveinfo.sound_end, qtrue );
		self->s.sound = 0;
	}
...
static void door_hit_bottom( edict_t *self )
{
	...
	{
		if( self->moveinfo.sound_end )
			G_AddEvent( self, EV_DOOR_HIT_BOTTOM, self->moveinfo.sound_end, qtrue );
		self->s.sound = 0;
	}
...
void door_go_down( edict_t *self )
{
	if( !( self->flags & FL_TEAMSLAVE ) )
	{
		if( self->moveinfo.sound_start )
			G_AddEvent( self, EV_DOOR_START_MOVING, self->moveinfo.sound_start, qtrue );
...
void SP_func_door( edict_t *ent )
{
	...
	// treat legacy door entities fine
	if( G_IsQ1Map() )
	{
		// disable gay opening by fire
		if( !ent->health )
			ent->health = -1;

		// don't want to link this door
		if( ent->spawnflags & Q1_DOOR_DONT_LINK )
		{
			ent->team = ent->model;
			ent->teammaster = ent;
		}

		ent->spawnflags &= ~(Q1_DOOR_DONT_LINK|Q1_DOOR_GOLD_KEY|Q1_DOOR_SILVER_KEY);
	}
	else if( G_IsQ2Map() )
	{
		if( !ent->health )
...
{
	return (ent->use == door_use) ? qtrue : qfalse;
}
...
static void button_fire( edict_t *self )
{
	...
	if( self->moveinfo.sound_start && !( self->flags & FL_TEAMSLAVE ) )
		G_AddEvent( self, EV_BUTTON_FIRE, self->moveinfo.sound_start, qtrue );
...
static void train_wait( edict_t *self )
{
	...
	if( self->moveinfo.wait )
	{
		...
		{
			if( self->moveinfo.sound_end )
				G_AddEvent( self, EV_TRAIN_STOP, self->moveinfo.sound_end, qtrue );
			self->s.sound = 0;
		}
	}
	else
...
again:
	...
	{
		if( self->moveinfo.sound_start )
			G_AddEvent( self, EV_TRAIN_START, self->moveinfo.sound_start, qtrue );
		self->s.sound = self->moveinfo.sound_middle;
	}
