...
static qboolean G_ItemTimerNeeded( gsitem_t *it )
{
	assert( it );
	...
	if( it->type == IT_POWERUP )
		return qtrue;
	if( it->type == IT_ARMOR )
		return ( it->tag == ARMOR_YA || it->tag == ARMOR_RA ) ? qtrue : qfalse;
	if( it->type == IT_HEALTH )
		return ( it->tag == HEALTH_MEGA || it->tag == HEALTH_ULTRA ) ? qtrue : qfalse;

	return qfalse;
}

static qboolean G_ItemTimerUnimportant( gsitem_t *it )
{
	assert( it );

	if( it->type == IT_ARMOR )
		return ( it->tag == ARMOR_GA ) ? qtrue : qfalse;
	return qfalse;
}
